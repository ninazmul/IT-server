...
exports.updatePassword = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const userId = req.user?._id?.toString(); // Ensure userId is string type
        await redis_1.redis.set(userId, JSON.stringify(user)); // Use userId as RedisKey type
...
exports.updateAvatar = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (avatar) {
            ...
            // Upload new avatar
            const uploadedAvatar = await cloudinary_1.default.v2.uploader.upload(avatar, {
                folder: "avatars",
                width: 150,
            });
...
exports.getAllUsersForAdmin = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
...
exports.updateUserRole = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const user = await user_model_1.default.findOne({ email });
        if (user) {
            await (0, user_service_1.updateUserRoleService)(user.id, role, res);
        }
        else {
            res.status(404).json({
                success: false,
